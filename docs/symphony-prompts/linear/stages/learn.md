...
Distill what this ticket taught into `docs/llm-wiki/` for the next operator.

1. Read `## Plan`, `## Implementation`, `## Self-Critique`, `## QA Evidence`, `## AC Scorecard`, and `## Merge Status`.
2. Compare brief vs reality: assumptions that held or broke, constraints that surfaced, prior wiki entries that were incomplete or misleading.
...
4. Append `## Learnings` -- 3-4 bullets of new facts, constraints, or surprises.
...
6. Append `## Human Review` with this shape:
   ...
   - `### Evidence` -- commands/proofs with pass/fail and top evidence path.
