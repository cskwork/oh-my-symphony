...
**Allowed tools (advisory).** Read full repo + docs + ticket. Write source files within the agreed Plan scope, `docs/{{ issue.identifier }}/work/`, and tracker comments. Run tests, formatters, `git add` / `git commit` (Symphony commits at turn end). Do NOT push branches, merge, or open PRs — Symphony's Learn Merge Gate handles integration.
