...
7. Prior findings resolved and nothing ≥ MEDIUM remains → do not post another Review Findings comment. Post a Review comment and pick the next state per step 9 in the same turn; staying in `Review` after a clean review is a workflow failure.
8. Only LOW findings (or none) → post a Review comment (header + the same severity table; flag deferred LOW items so Learn can address them), then pick the next state per step 9.
9. **Next state by difficulty** (clean review only — steps 7/8): Plan declared `## Difficulty: trivial` AND this issue did **not** change runtime behavior (the same condition that gates the live-HTTP probe in step 4) → transition state to `Learn` (skip QA). Otherwise → transition state to `QA` as today. Safety rails override difficulty: a `bug` issue may never skip QA (repro closure stays mandatory), and any `fail` Security Audit row forces the full route to `QA`. Post a one-line `## Pipeline Route` comment recording the routing decision so any skipped QA is never silent — e.g. `trivial, no runtime change → Learn (QA skipped)` or `runtime change → QA`.
10. Genuinely out of scope or unfixable → transition state to `Blocked`, post a Blocker comment with what is needed and stop.
