...
7. Append `## Difficulty` (right after `## Done Signals`) — exactly one of `trivial` / `standard` / `complex`, plus a one-line rationale (cap 2 lines total). This declares how much of the pipeline the ticket needs: `trivial` lets a non-`bug` ticket with no runtime-behavior change skip Critic and QA; `standard` / `complex` run the full loop. Omitting `## Difficulty` defaults to `standard`. Pick `trivial` only for a single-spot edit the visible tests already cover (e.g. a doc/comment fix, a string substitution, a metadata bump); when in doubt choose `standard`.
8. If you rejected any `reuse-inventory.md` row with `reuse_fit >= 0.7`: append `## Plan Rationale`, one line per rejected row (e.g. `path:line — reuse_fit 0.8 rejected: API shape mismatch`).
9. Set state to `In Progress`. In Progress must read this `## Plan` before editing code.
