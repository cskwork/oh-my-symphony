...
The release blockers are fixed and released as `v0.9.1`.
...
- Fresh r8 four-agent E2E from clean pushed `dev` reached `Human Review` for
  OpenCode, Pi, Claude, and Codex.
- `/api/v1/state` ended with `running=0` and `retrying=0`.
- Branch-archive browser gates passed for all four generated todo apps.
- `symphony service stop --force` closed the r8 service ports and left no
  process referencing the temp root.

Release-number reconciliation:

- Earlier notes treated `v0.8.1` as the likely next patch/minor tag from the
  old latest tag `v0.8.0`.
- Source files already declared `0.9.1`, so the final published release used
  `v0.9.1` to match `pyproject.toml` and `src/symphony/__init__.py`.
...
At the time of this triage, the latest local release tag was `v0.8.0` and the
next-release number still needed reconciliation after the blockers below were
fixed and the full gate was green.
...
source files. Final reconciliation selected `v0.9.1` because both source files
already declared `0.9.1`.
...
7. JavaScript syntax or unit checks pass where generated by the worker; the
   authoritative UI proof is the real Playwright browser gate above.
...
5. Commit the release bump as `chore(release): v0.9.1`.
6. Tag `v0.9.1` on the release commit.
...
- Rejected: tag before `dev` and `main` are green. That would publish
