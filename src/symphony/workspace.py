...
async def commit_workspace_on_done(
    ...
    *,
    ...
    script = (
        ...
        '  # uncommitted changes into one. --mixed rebuilds the index from\n'
        '  # the base first so excluded paths from prior wip commits do not\n'
        '  # survive as stale staged entries in the final ticket commit.\n'
        '  git reset --mixed "$BASE" || exit 44\n'
        'fi\n'
